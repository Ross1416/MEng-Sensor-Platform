...
def calculate_distance(objects, distance_moved, last_objects):
    ...
    returns: updated object array with distances to objects added
    """
        ...
        for j, obj in enumerate(camera):
            ...
            for k, last_obj in enumerate(last_objects[i]):
                ...
                if obj.label == last_obj.label:
                    ...
                    if i % 2 == 0:
                        last_height = last_obj.get_xywh()[3]
                        height = obj.get_xywh()[3]
                        distance = longitudinal_depth(
                            distance_moved, last_height, height
                        )
                        obj.distance = distance
                    # Calculate lateral distance for side camera distance
                    else:
                        last_x = (last_obj.get_xyxy()[0] + last_obj.get_xyxy()[2]) / 2
                        x = (obj.get_xyxy()[0] + obj.get_xyxy()[2]) / 2
...
if __name__ == "__main__":
    ...
    detection_img1 = cv2.rectangle(
        detection_img1, results1[0][1][:2], results1[0][1][2:], (255, 0, 0), 1
    )
    detection_img2 = cv2.rectangle(
        detection_img2, results2[0][1][:2], results2[0][1][2:], (255, 0, 0), 1
    )
